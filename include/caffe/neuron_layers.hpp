...
/**
 * @brief During training only, replaces each pixel with a random pixel
 * 		from a neighborhood of the specified radius.
 *
 * @param bottom input Blob vector (length 1)
 *   -# @f$ (N \times C \times H \times W) @f$
 *      the inputs @f$ x @f$
 ...
 */
template <typename Dtype>
class JitterLayer : public NeuronLayer<Dtype> {
 public:
  /**
   * @param param provides JitterParameter jitter_param,
   *     with jittering option:
   *   - jitter_radius (\b optional, default 3).
   */
  explicit JitterLayer(const LayerParameter& param)
      : NeuronLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "Jitter"; }
