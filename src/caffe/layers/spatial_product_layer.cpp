...
void SpatialProductLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int n = 0; n < num_; ++n) {
      const Dtype* scale_data  = bottom[1]->cpu_data(n);
      for (int c = 0; c < channels_; ++c) {
        Dtype* top_data          = top[0]->mutable_cpu_data(n, c);
        const Dtype* bottom_data = bottom[0]->mutable_cpu_data(n, c);
...
void SpatialProductLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[1]) {
    for (int n = 0; n < num_; ++n) {
      ...
      Dtype* scale_diff        = bottom[1]->mutable_cpu_diff(n);
