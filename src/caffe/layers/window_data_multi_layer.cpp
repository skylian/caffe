...
WindowDataMultiLayer<Dtype>::~WindowDataMultiLayer<Dtype>() {
	this->JoinPrefetchThread();
}

template<typename Dtype>
void WindowDataMultiLayer<Dtype>::DataLayerSetUp(
		const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
	// LayerSetUp runs through the window_file and creates one structure
	// that hold windows for foreground objects
	...
	//    width
	//    num_windows
	//    x1 y1 x2 y2 K(dim of label vector) L1 L2...LK

	LOG(INFO)<< "Window data layer:" << std::endl
	<< "  cache_images: "
	<< this->layer_param_.window_data_multi_param().cache_images() << std::endl
	<< "  root_folder: "
	<< this->layer_param_.window_data_multi_param().root_folder();

	cache_images_ = this->layer_param_.window_data_multi_param().cache_images();
	string root_folder = this->layer_param_.window_data_multi_param().root_folder();
	...
	std::ifstream infile(this->layer_param_.window_data_multi_param().source().c_str());
	CHECK(infile.good()) << "Failed to open window file "
	<< this->layer_param_.window_data_multi_param().source() << std::endl;
	...
	do {
		...
		for (int i = 0; i < num_windows; ++i) {
			int x1, y1, x2, y2, dim_label;
			infile >> x1 >> y1 >> x2 >> y2 >> dim_label;
			CHECK( dim_label_ < 0 || dim_label_ == dim_label );
			dim_label_ = dim_label;

			vector<float> window(WindowDataMultiLayer::LABEL_START+dim_label);
			window[WindowDataMultiLayer::IMAGE_INDEX] = image_index;
			window[WindowDataMultiLayer::X1] = x1;
			window[WindowDataMultiLayer::Y1] = y1;
			window[WindowDataMultiLayer::X2] = x2;
			window[WindowDataMultiLayer::Y2] = y2;
			for (int k = 0; k < dim_label; ++k) {
				infile >> window[WindowDataMultiLayer::LABEL_START + k];
			}
	...
	<< this->layer_param_.window_data_multi_param().context_pad();

	LOG(INFO) << "Crop mode: "
	<< this->layer_param_.window_data_multi_param().crop_mode();

	// image
	const int crop_size = this->transform_param_.crop_size();
	CHECK_GT(crop_size, 0);
	const int batch_size = this->layer_param_.window_data_multi_param().batch_size();
...
template<typename Dtype>
unsigned int WindowDataMultiLayer<Dtype>::PrefetchRand() {
	CHECK(prefetch_rng_);
...
template<typename Dtype>
void WindowDataMultiLayer<Dtype>::InternalThreadEntry() {
	...
	const Dtype scale = this->layer_param_.window_data_multi_param().scale();
	const int batch_size = this->layer_param_.window_data_multi_param().batch_size();
	const int context_pad =
			this->layer_param_.window_data_multi_param().context_pad();
	const int crop_size = this->transform_param_.crop_size();
	...
	for (int dummy = 0; dummy < batch_size; ++dummy) {
		...
		pair<std::string, vector<int> > image =
				image_database_[window[WindowDataMultiLayer < Dtype > ::IMAGE_INDEX]];

		cv::Mat cv_img;
		if (this->cache_images_) {
			pair<std::string, Datum> image_cached =
					image_database_cache_[window[WindowDataMultiLayer < Dtype
							> ::IMAGE_INDEX]];
		...
		int x1 = window[WindowDataMultiLayer < Dtype > ::X1];
		int y1 = window[WindowDataMultiLayer < Dtype > ::Y1];
		int x2 = window[WindowDataMultiLayer < Dtype > ::X2];
		int y2 = window[WindowDataMultiLayer < Dtype > ::Y2];
			...
			for (int w = 0; w < cv_cropped_img.cols; ++w) {
				for (int c = 0; c < channels; ++c) {
					int top_index = ((item_id * channels + c) * crop_size + h
							+ pad_h) * crop_size + w + pad_w;
					// int top_index = (c * height + h) * width + w;
					Dtype pixel = static_cast<Dtype>(ptr[img_index++]);
					if (this->has_mean_file_) {
						int mean_index =
								(c * mean_height + h + mean_off + pad_h)
		...
		<< window[WindowDataMultiLayer<Dtype>::X1]+1 << std::endl
		<< window[WindowDataMultiLayer<Dtype>::Y1]+1 << std::endl
		<< window[WindowDataMultiLayer<Dtype>::X2]+1 << std::endl
		<< window[WindowDataMultiLayer<Dtype>::Y2]+1 << std::endl
		<< do_mirror << std::endl;
		for (int k = 0; k < dim_label_; ++k)
			inf << top_label[item_id*dim_label_+k] << " ";
		inf << std::endl;
		inf.close();
		LOG(INFO) << (string("dump/") + file_id + string("_data.txt")).c_str();
...
INSTANTIATE_CLASS(WindowDataMultiLayer);
REGISTER_LAYER_CLASS(WindowDataMulti);
